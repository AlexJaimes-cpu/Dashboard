...
try:
    ...
    with col2:
        st.markdown("### Top 5 en Puntos de Venta")
        fig, axs = plt.subplots(1, len(puntos_venta), figsize=(15, 5))
        for i, punto in enumerate(puntos_venta):
            top_punto_venta = datos.groupby("nombre")[f"{punto} vendido"].sum().nlargest(5)
            top_punto_venta.plot(kind="pie", autopct='%1.1f%%', ax=axs[i], startangle=90, legend=False)
            axs[i].set_title(punto.title())
            axs[i].set_ylabel("")
        st.pyplot(fig)
